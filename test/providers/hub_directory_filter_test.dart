...
void main() {
  group('loadDirectory location filters (ADR-035 Phase 2)', () {
    ...
    test(
      'uppercases the country code so the picker output is canonical',
      () async {
        SharedPreferences.setMockInitialValues({});
        AuthService.storage = MockSecureStorage();
        final ffi = _MockFfiService();
    ...
    test(
      'exposes hasActiveLocationFilter when at least one filter is set',
      () async {
    ...
    test(
      'preserves existing filters when reloading with only a search',
      () async {
        SharedPreferences.setMockInitialValues({});
        AuthService.storage = MockSecureStorage();
        final ffi = _MockFfiService();
    ...
    test(
      'clearLocationFilter wipes both country and cityId in one call',
      () async {
        SharedPreferences.setMockInitialValues({});
        AuthService.storage = MockSecureStorage();
        final ffi = _MockFfiService();
